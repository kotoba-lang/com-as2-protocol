(ns as2_protocol.main
  "Kotodama WASM entrypoint for the As2_protocol clean-room actor (L4) — Clojure port.
  ...
  facts (`as2_protocol.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "as2_protocol")
(def tier "L4")
...
(def entity-specs
  [{:entity "As2Message"   :plural "as2messages"   :id-prefix "as2proto_as2"
    :fields [:messageId :as2From :as2To :as2Version :contentType :micalg :contentTransferEncoding :dispositionNotificationTo] :required [:as2From :as2To]
    :coerce {}                       :refs {}}
   {:entity "Mdn"      :plural "mdns"      :id-prefix "as2proto_mdn"
    :fields [:messageId :originalMessageId :finalRecipient :dispositionType :dispositionMode :actionMode :sendingMode :dispositionModifier :digestAlgorithm] :required [:finalRecipient :dispositionType]
    :coerce {}          :refs {}}
   {:entity "DispositionNotification"     :plural "dispositionnotifications"     :id-prefix "as2proto_dis"
    :fields [:reportingUa :finalRecipient :originalMessageId :disposition :receivedContentMic]        :required [:reportingUa :finalRecipient]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"as2_protocol.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "as2_protocol-compat" :tier tier :entities entities} 200])
...
(defn start! [] :as2_protocol-compat/ready)
